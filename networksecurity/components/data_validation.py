...
class DataValidation:
    ...
    def validate_numerical_columns(self, dataframe: pd.DataFrame) -> bool:
        try:
            required_numerical_columns = read_yaml_file(SCHEMA_FILE_PATH)
    ...
    def initiate_data_validation(self)-> DataValidationArtifact:
        try:
            ...
            if not status_train:
                error_message = f"{error_message} Train dataframe does not constain all colummns. \n "
            status_test = self.validate_number_of_columns(dataframe=test_dataframe)
            if not status_test:
                error_message = f"{error_message} Test dataframe does not constain all colummns. \n "
